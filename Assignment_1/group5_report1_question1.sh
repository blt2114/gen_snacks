...
pass_2d=`sudo poretools fasta --type 2D $pass_dir | wc -l`
...
# forward pass reads.
pass_fwd=`sudo poretools fasta --type fwd $pass_dir | wc -l`
...
fail_2d=`sudo poretools fasta --type 2D $fail_dir | wc -l`
fail_2d=`echo $fail_2d / 2 | bc`
echo "number of failed 2D: $fail_2d"

fail_fwd=`sudo poretools fasta --type fwd $fail_dir | wc -l`
